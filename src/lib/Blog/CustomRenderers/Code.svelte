<script>
	...
	const html = marked
		.walkTokens(marked.lexer(raw), (token) => {
			const lang = token.lang.split(' ')[0] ?? 'txt';
			// console.log(lang)
			const shouldHighlightLine = calculateLinesToHighlight(lang);
			return text.split('\n').map((line, i) => {
...
<pre class="bg-slate-900 py-4 px-2 my-4 text-gray-50 rounded-lg overflow-x-auto text-base"><code>{@html html}</code></pre>
